...
/**
 * The real-file counterpart of [MultipedeGadgetXorSoundnessTest]: does adding the
 * cluster-generalized flip-parity XOR clause -- validated against real automorphisms in
 * [RealFileFlipParityValidationTest] -- keep the encoding sound on an ACTUAL cfi-rigid-r2 file
 * (not just the from-scratch construction)? Checked on both:
 * - cfi-rigid-r2-0068-03-1: the smallest file with a non-trivial Aut(G), so orbits are non-trivial
 *   and there's something for the XOR clause to actually interact with.
 * - cfi-rigid-r2-0072-01-1: a RIGID file (the overwhelmingly common case in this family) -- proves
 *   the XOR clause does not introduce a false UNSAT/wrong-orbit result on the case that actually
 *   dominates the benchmark.
 */
class RealFileGadgetXorSoundnessTest {
    ...
    private fun compare(path: String) {
        ...
        val xorResult = try {
            val clustersConstrained = RealFileGadgetXor.addClusterParityXors(xorCmsSolver, xorCmsEncoding, sided)
    ...
    @Test
    fun soundOnSmallestNonRigidR2File() {
        compare("graphs/cfi-rigid-r2/cfi-rigid-r2-0068-03-1")
    }

    @Test
    fun soundOnARigidR2File() {
        compare("graphs/cfi-rigid-r2/cfi-rigid-r2-0072-01-1")
    }

    @Test
    fun payoffScalingSweep() {
        for (path in listOf(
            "graphs/cfi-rigid-r2/cfi-rigid-r2-0432-01-1",
            "graphs/cfi-rigid-r2/cfi-rigid-r2-1008-01-1",
        )) compare(path)
    }
}
